...
def create_app(config_class=Config):
    ...
    cors.init_app(
        app,
        resources={
            r"/*": {"origins": app.config.get("FRONTEND_ORIGIN", "http://localhost:3000"), "supports_credentials": True}
        },
    )
    # HTTP security headers via Flask-Talisman
    # Check if testing to disable strict HTTPS enforcement during tests
    ...
    talisman.init_app(
        ...
        content_security_policy={
            "default-src": ["'self'"],
            "script-src": ["'self'"],
            "connect-src": ["'self'", app.config.get("FRONTEND_ORIGIN", "http://localhost:3000")],
            "img-src": ["'self'"],
...
# Global instance for backward compatibility (e.g. if anything imports 'app' directly)
app = create_app()
